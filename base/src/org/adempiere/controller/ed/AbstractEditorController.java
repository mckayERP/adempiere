...
/**
 * An abstract controller of view editors for specific fields. The controller
 * manages the interactions with the model in a consistent way across all view
 * types.
 * 
...
public abstract class AbstractEditorController implements EditorController {
    ...
    /**
     * The last display value. The text displayed can change without the
     * underlying value changing so this variable provides a means to test if a
     * change has occurred.
     */
    protected String lastDisplay;

    /**
     * Standard constructor where the editor is not bound to a tab or model
     * field
     */
    ...
    /**
     * Standard constructor for testing.
     */
    public AbstractEditorController(CEditor editor) {
    ...
    /**
     * Standard constructor where the editor is in a tab but the model field has
     * not been identified yet.
     * 
     * @param tab      - the model GridTab controlling the field
     * @param windowNo - the window number for this editor
     */
    public AbstractEditorController(CEditor editor, GridTab tab,
            boolean mandatory, boolean isReadOnly,
    ...
    /**
     * Private constructor where the editor is in a tab but the model field has
     * not been identified yet. The editor has to be defined by the public
     * constructors
     * 
     * @param tab      - the model GridTab controlling the field
     * @param field    - the model GridField for this editor
     * @param windowNo - the window number for this editor
     */
    private AbstractEditorController(GridTab tab, GridField field) {
    ...
    public void setGridField(GridField field) {
        ...
        if (gridField != null) {
            ...
            columnId = gridField.getAD_Column_ID();
    ...
    public boolean hasChanged() {
        ...
        // Both the current or oldValue could be null
        if (getValue() != null) {
    ...
    public void generateZoomQuery() {
        ...
        if (zoomKeyTableName == null || zoomKeyTableName.isEmpty()
            ...
            throw new AdempiereException("Can't zoom. Unknown zoom table "
                    + "and key column");
        ...
        zoomQuery.addRestriction(zoomKeyColumnName, MQuery.EQUAL, currentValue);
        zoomQuery.setZoomColumnName(zoomKeyColumnName);
        zoomQuery.setZoomTableName(zoomKeyTableName);
        zoomQuery.setZoomValue(currentValue);
        zoomQuery.setRecordCount(1); // guess

        zoomWindowId = lookup.getZoom(zoomQuery);
    ...
    protected void setZoomAD_Window_ID(int zoomAD_Window_ID) {
        ...
        this.zoomWindowId = zoomAD_Window_ID;
    ...
    public void setEditor(CEditor editor) {
