...
//			}
	...
	private boolean saveSelection()
	{
		...
		if (!isProductWindow && !as.excludeEntry(MColumn.getTable_ID(Env.getCtx(), m_AD_Column_ID, null), m_isSOTrx)) {
...
//			}
		...
		if (productASI != null 
			...
			matchingASI_ID = findMatchingAttributeSetInstance(Env.getCtx(), instanceASI, values, null);  // Will return 0 if nothing found.
		}
		...
		if (instanceASI.getM_AttributeSetInstance_ID() >= 0 && matchingASI_ID == 0) {
			...
			// This should never happen as findMatchingAttributeSetInstance(asi, values) filters out such a match.
		...
		//  Save attributes
		//  m_readWrite is true
		if (m_attributeSetInstance_id > 0 && !hasValues(instanceASI, values)) {
